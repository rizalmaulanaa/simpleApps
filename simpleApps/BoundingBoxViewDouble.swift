...
struct BoundingBoxViewDouble: View {
    let image: Image
    let imageUI: UIImage
    let boundingBoxes1: [[Float]] // Array of arrays of Int representing bounding boxes
    let boundingBoxes2: [[Float]] // Array of arrays of Int representing bounding boxes
    let imageShape: [Int]
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack (alignment: .bottom) {
                ...
                ForEach(Array(self.boundingBoxes1.enumerated()), id: \.offset) { index, boundingBox in
                ...
                ForEach(Array(self.boundingBoxes2.enumerated()), id: \.offset) { index, boundingBox in
                    let fixX = self.width * CGFloat(boundingBox[0])
                    let fixY = self.height * CGFloat(boundingBox[1]) + self.additionY
                    let fixW = self.width * CGFloat(boundingBox[2])
                    let fixH = self.height * CGFloat(boundingBox[3])
